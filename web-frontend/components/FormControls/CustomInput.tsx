...
const CustomInput = ({ type, placeHolder, value, required, name, label, handleChange }: CustomInputProps) => {
  return (
    <figure className='flex flex-col space-y-2'>
      ...
      <input type={type} placeholder={placeHolder} value={value} onChange={handleChange} required={required} name={name} className='py-3 px-4 rounded-md text-xs font-medium text-gray-900 border border-ascent-dark border-opacity-30 bg-indigo-50 bg-opacity-20 w-full placeholder:text-black placeholder:font-normal placeholder:text-opacity-50' />
    </figure>
  )
}
